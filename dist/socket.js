...
exports.setupSocket = exports.broadcastStaffStatus = void 0;
...
// ────────────────────────────────────────────────────────────────
const onlineStaff = new Map(); // socket.id → userId   (staff can have only ONE active socket)
const onlineStudents = new Set(); // userId               (students: simple presence)
// ────────────────────────────────────────────────────────────────
// UTILITIES
// ────────────────────────────────────────────────────────────────
const getRoom = (a, b) => `private_${[a, b].sort().join("_")}`;
// ────────────────────────────────────────────────────────────────
// BROADCAST: Send full staff list + who is online to EVERYONE
// ────────────────────────────────────────────────────────────────
const broadcastStaffStatus = async (io) => {
    try {
...
exports.broadcastStaffStatus = broadcastStaffStatus;
// ────────────────────────────────────────────────────────────────
// MAIN SOCKET SETUP
// ────────────────────────────────────────────────────────────────
const setupSocket = (io) => {
    io.on("connection", async (socket) => {
        ...
        // ────────────────────────────────────────────────────────
        const incomingId = socket.handshake.query.userId;
        let userId = null;
        let role = null;
        let idNumber = null;
        try {
        ...
        await (0, exports.broadcastStaffStatus)(io);
        if (role === "staff") {
            try {
        ...
        socket.on("open_private_chat", async (targetUserId) => {
            ...
            const messagesWithDetails = await Promise.all(history.map(async (m) => {
        ...
        socket.on("send_private_message", async ({ text, to }) => {
            if (!text.trim() || !userId || !to || !role)
                return;
            try {
        ...
        socket.on("staff_request_conversations", async () => {
            if (socket.data.role !== "staff" || !socket.data.userId)
                return;
        ...
        socket.on("disconnect", (reason) => {
            ...
            // Update everyone
            (0, exports.broadcastStaffStatus)(io);
        });
